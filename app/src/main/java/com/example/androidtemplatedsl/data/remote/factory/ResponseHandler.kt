        ...
        return if (response.isSuccessful) {
            if (body != null) {
                ...
                if (successBodyType == Unit::class.java) {
                    ...
                    NetworkResponse.Success(Unit) as NetworkResponse<S, E>
                } else {
                    NetworkResponse.Error(body = null,code =  code,type = NetworkResponse.ErrorType.SERVER,error = null)
                }
            }
        } else {
            val networkResponse: NetworkResponse<S, E> = try {
                ...
                NetworkResponse.Error(body = convertedBody,code =  code,type = NetworkResponse.ErrorType.SERVER,error = null)
            } catch (ex: Exception) {
                NetworkResponse.Error(body = null,code =  code,type = NetworkResponse.ErrorType.UNKNOWN,error = ex)
            }
            networkResponse
        }
    }
